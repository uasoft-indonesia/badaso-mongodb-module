...
class ContentManager
{
    ...
    {
        $collection_name = $data_type->getTable();
    ...
    private function populateInsertStatements(
        ...
        $inserts = '';
        $inserts .= sprintf(
            "\DB::connection('mongodb')->collection('%s')->insert(%s);",
            $table_name,
    ...
    public function populateCollectionContentToSeeder(string $stub, string $collection_name, array $data)
    {
        return $this->populateInsertStatements($stub, $collection_name, $data, '{{insert_statements}}');
    }
}
